...
# -----------------------------------------------------------------------------
with open("experiments/exp006_hill/exp/001.yaml") as f:
...
def optimize_grid_config(args: tuple) -> tuple[int, float, list[tuple[float, float, float]]]:
    ...
    best_score, best_xs, best_ys, best_degs, best_a, best_b = sa_optimize_improved(
        ...
        params["delta_t"],
        seed,
        n_trees,  # Pass N for logging inside numba
    )
    ...
    print(f"[PID {os.getpid()}] Finished: N={n_trees}, Score={best_score:.5f}")
...
if __name__ == "__main__":
    print("=" * 80)
    print("Improved SA Translation Optimizer (Numba-accelerated) with Logging")
    print("=" * 80)
    ...
    print(f"Saved to {out_path} (Always updated)")

    # Overlap Script Execution
    script_path = CONFIG["paths"]["overlap_script"]
    if os.path.exists(script_path):
        print("Running overlap correction script...")
        cmd = f"python {script_path} {baseline_path} {out_path}"
        os.system(cmd)

    if final_score >= baseline_total:
        print("Note: Total score did not improve, but file was updated.")
