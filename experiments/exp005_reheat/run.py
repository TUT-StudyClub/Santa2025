...
def sa_optimize_improved(  # noqa: PLR0912, PLR0913, PLR0915
    ...
    delta_t,
    random_seed,
    n_cycles=3,  # <--- デフォルト値を追加して呼び出しやすくしました
):
    """
    SAによる最適化 (Reheating + Early Stopping + Decay)
    """
    ...
    # --- Reheating & Early Stopping 設定 ---
    steps_per_cycle = nsteps // n_cycles
    t_factor = -math.log(t_max / t_min)
    patience = nsteps // 5  # 全体の20%更新なければ停止
    no_improve_count = 0
    ...
    for step in range(nsteps):
        ...
        # 1. Reheating: サイクルごとに温度をリセット
        temp = t_max * math.exp(t_factor * step_in_cycle / steps_per_cycle)

        # 2. Decay: サイクル内でステップ幅を減衰 (100% -> 10%)
        progress_in_cycle = step_in_cycle / steps_per_cycle
        decay = 1.0 - 0.9 * progress_in_cycle

        cur_pos_delta = position_delta * decay
        cur_ang_delta = angle_delta * decay
        cur_ang_delta2 = angle_delta2 * decay
        cur_delta_t = delta_t * decay
        ...
        # サイクル開始時はカウンタをリセット (再加熱直後は探索させたいため)
        if step_in_cycle == 0:
            no_improve_count = 0

        # 我慢の限界を超えたら、このサイクルはもう諦めて次のサイクルへ (実質スキップ)
        # ※シンプルに break すると全終了してしまうため、サイクル終了判定を行う
