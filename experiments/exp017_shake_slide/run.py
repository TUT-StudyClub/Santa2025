"""
exp017_shake_slide: 揺らし + 方向スライド最適化

小さな揺らしで配置の局所最適から抜けつつ、方向スライドで隙間を詰める。
"""
...
def resolve_config_path() -> str:
    ...
    return os.path.join("experiments", "exp017_shake_slide", "exp", f"{config_name}.yaml")
...
def jitter_group(
    ...
    for idx in indices:
        ...
        for _ in range(per_tree_tries):
            ...
            if not has_overlap_for_tree(int(idx), new_verts, polygons):
                xs[idx] = new_x
                ys[idx] = new_y
                degs[idx] = new_deg
                polygons[idx] = new_verts
                moved = True
                break

        if not moved:
            polygons[idx] = old_verts
...
if __name__ == "__main__":
    print("揺らしスライド最適化 (exp017_shake_slide)")
    ...
    for n in tqdm(range(n_min, n_max + 1), desc="最適化"):
        ...
        polygons = build_vertices(xs, ys, degs)
        orig_score, orig_side = calculate_score(polygons)

        if shake_enabled:
            cell_size = orig_side / math.sqrt(n)
            jitter_radius = jitter_radius_ratio * cell_size
            xs, ys, degs = jitter_group(xs, ys, degs, jitter_radius, jitter_deg, per_tree_tries, rng)
